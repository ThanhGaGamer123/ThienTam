...
public class customer_BUS {
    ...
    public static void findAdvance(DefaultTableModel model_kh, JTextField makh,
        ...
        if (!makh.getText().isEmpty())
            condition.add("makh like N'%" + makh.getText() + "%' ");
        if (!tenkh.getText().isEmpty())
            condition.add("tenkh like N'%" + tenkh.getText() + "%' ");
        if (!sdt.getText().isEmpty())
            condition.add("sdt like N'%" + sdt.getText() + "%' ");
        if (!email.getText().isEmpty())
            condition.add("email like N'%" + email.getText() + "%' ");
    ...
    public static void loadTable(DefaultTableModel table) {
        ...
        for (customer_DTO kh : dao.selectAll()) {
            ...
            table.addRow(new Object[] { kh.getMakh(), kh.getTenkh(), kh.getSdt(), kh.getEmail(),
                    diaChi, kh.getPassword(), kh.getDiemKM(), statusImg });
        }
    }
    ...
    public static ArrayList<customer_DTO> getAll() {
        customer_DAO dao = new customer_DAO();
